...
namespace PPM.MAIN.PPM.DOMAIN
{
    public class ProjectManager
    {
        ...
        public void Display()
        {
            ...
            {
                if(projects[j].addingemployeeList.Count == 0)
                {
                ...
                else
                {
                    ...
                    for(int i=0; i<projects[j].addingemployeeList.Count; i++)
                    {
                        Console.WriteLine("-------------------------------------------");
                        Console.WriteLine(projects[j].addingemployeeList[i].employeefirstName+" ["+projects[j].addingemployeeList[i].roleName + "]");
                    } 
                }
            }
        }
        ...
        public void EmployeeToproject(int pid, Employee ename)
        {
            if(!EM.Exist(pid))
            {
                for(int i=0; i<projects.Count; i++)
                {
                    ...
                    {
                        projects[i].addingemployeeList.Add(ename);
                    }
                }
            }
        ...
        public void EmployeeFromProject(int pid, Employee ename)
        {
            for(int i=0; i<projects.Count; i++)
            {
                if(projects[i].id==pid)
                {
                    projects[i].addingemployeeList.Remove(ename);
                }
            }
        }
